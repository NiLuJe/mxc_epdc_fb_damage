    ...
    init_module(void)
{
	...
	orig_disp_fops                   = fops_get(disp_cdev->ops);
	patched_disp_fops                = *orig_disp_fops;
	patched_disp_fops.unlocked_ioctl = disp_ioctl;
	// NOTE: The replace_fops macro is designed for a struct file*, not a struct cdev*...
	fops_put(disp_cdev->ops);
	BUG_ON(!(disp_cdev->ops = &patched_disp_fops));
